    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col-md-12" style="text-align: center ;">
                <h2>เลื่อนวันนัดรับ - นัดคืนเครื่องประดับ</h2>
            </div>
        </div>
        <div class="row mt-5">
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        var calendar = new FullCalendar.Calendar(calendarEl, {
                            ...
                            events: [
                                ...
                                @foreach ($reservation_jewelry_total as $reservation)
                                    {
                                        @php
            ...
            <div class="col-md-5">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($reservation_jewelry_total as $index => $item)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    @php
        ...
        <div class="row mt-5 mb-5">
            <div class="col-md-12">
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <p>{{ $typejewelry->type_jewelry_name }}
                        ...
                        <form action="{{ route('postponeroutejewelrycheckednoset', ['id' => $reser->id]) }}"
                            method="GET">
                            @csrf
                        ...
                        <h5>เงื่อนไขการเลื่อนวันนัดรับ-นัดคืนเครื่องประดับ</h5>
                        <p>
                            การเลื่อนวันนัดรับ-นัดคืนเครื่องประดับจะต้องผ่านการตรวจสอบความพร้อมของเครื่องประดับทั้ง 3
                            เงื่อนไขดังต่อไปนี้:
                        </p>
                        <ul>
                            <li>
                                <strong>เงื่อนไขที่ 1 (ช่วงก่อนวันนัดรับใหม่):</strong> ระบบจะตรวจสอบ 7
                                วันก่อนวันนัดรับใหม่ว่าเครื่องประดับไม่ติดการจองหรือการเช่าของลูกค้าท่านอื่น
                            </li>
                            <li>
                                <strong>เงื่อนไขที่ 2 (ช่วงวันนัดรับใหม่ถึงวันนัดคืนใหม่):</strong>
                                ระบบจะตรวจสอบว่าในช่วงวันที่ต้องการเลื่อนไปใช้บริการนั้น
                                เครื่องประดับไม่ติดการจองหรือการเช่าของลูกค้าท่านอื่น
                            </li>
                            <li>
                                <strong>เงื่อนไขที่ 3 (ช่วงหลังวันนัดคืนใหม่):</strong> ระบบจะตรวจสอบ 7
                                วันหลังวันนัดคืนใหม่ว่าเครื่องประดับไม่ติดการจองหรือการเช่าของลูกค้าท่านอื่น
                            </li>
                        </ul>
                        <p>
                            <strong>หมายเหตุ:</strong> การเลื่อนวันนัดรับ-นัดคืนจะสามารถทำได้ก็ต่อเมื่อผ่านทั้ง 3
                            เงื่อนไขข้างต้น
                            เพื่อให้มั่นใจว่ามีช่วงเวลาเพียงพอสำหรับการเตรียมเครื่องประดับก่อนส่งมอบและการจัดการหลังคืนเครื่องประดับ
        ...
        <div class="modal fade" id="rescheduleModal" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="{{ route('postponecheckedpassjewelry', ['id' => $orderdetail->id]) }}" method="POST">
